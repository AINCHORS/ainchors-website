...
@section('content')
    <div class="mb-8">
        ...
        <p class="mt-2 max-w-3xl text-sm leading-relaxed text-ainchors-grey-dark">Grant and revoke course access without creating duplicate enrollment records. Every manual access change requires an audit reason.</p>
    </div>
    ...
    <details class="mb-6 rounded-ainchors-card border border-ainchors-navy/10 bg-white shadow-sm" @if($errors->hasAny(['user_id', 'product_id', 'expires_at', 'reason'])) open @endif>
        ...
        <form method="POST" action="{{ route('admin.enrollments.store') }}" class="grid gap-5 border-t border-ainchors-navy/10 p-5 sm:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_12rem_minmax(14rem,1fr)_auto] xl:items-end">
            ...
            <div>
                ...
                <select id="enrollment-user" name="user_id" required class="mt-2 block w-full rounded-ainchors-button border border-ainchors-grey-light/45 bg-white px-3.5 py-2.5 text-sm text-ainchors-navy outline-none transition focus:border-ainchors-green focus:ring-2 focus:ring-ainchors-green/25">
                    <option value="">Select a user</option>
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" @selected((string) old('user_id') === (string) $user->id)>{{ $user->full_name }} — {{ $user->email }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="enrollment-course" name="product_id" required class="mt-2 block w-full rounded-ainchors-button border border-ainchors-grey-light/45 bg-white px-3.5 py-2.5 text-sm text-ainchors-navy outline-none transition focus:border-ainchors-green focus:ring-2 focus:ring-ainchors-green/25">
                    <option value="">Select a course</option>
                    @foreach ($courses as $course)
                        <option value="{{ $course->id }}" @selected((string) old('product_id') === (string) $course->id)>{{ $course->name }}</option>
                    @endforeach
                </select>
    ...
    <form method="GET" action="{{ route('admin.enrollments.index') }}" class="mb-6 grid gap-3 rounded-ainchors-card border border-ainchors-navy/10 bg-white p-4 shadow-sm sm:grid-cols-[minmax(0,1fr)_10rem_auto] sm:items-end">
        <div>
            <label for="enrollments-search" class="block text-sm font-semibold text-ainchors-navy">Search</label>
            <input id="enrollments-search" name="q" type="search" value="{{ request('q') }}" placeholder="User, email or course" class="mt-2 block w-full rounded-ainchors-button border border-ainchors-grey-light/45 px-3.5 py-2.5 text-sm outline-none transition focus:border-ainchors-green focus:ring-2 focus:ring-ainchors-green/25">
        </div>
        <div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.enrollments.index') }}" class="rounded-ainchors-button border border-ainchors-navy/15 px-4 py-2.5 text-sm font-semibold text-ainchors-grey-dark transition hover:border-ainchors-green hover:text-ainchors-green focus:outline-none focus:ring-2 focus:ring-ainchors-green">Reset</a>
        </div>
    </form>
            ...
            <table class="w-full min-w-[76rem] text-left text-sm">
                ...
                <tbody class="divide-y divide-ainchors-navy/8">
                    @forelse ($enrollments as $enrollment)
                        @php($revokeFormId = 'revoke-enrollment-'.$enrollment->id)
                        <tr>
                            ...
                            @if ($enrollment->status !== 'revoked')
                                <td class="px-5 py-4">
                                    <label class="sr-only" for="revoke-reason-{{ $enrollment->id }}">Reason for revoking enrollment</label>
                                    <input id="revoke-reason-{{ $enrollment->id }}" form="{{ $revokeFormId }}" name="reason" type="text" maxlength="500" required placeholder="Reason required" class="w-52 rounded-ainchors-button border border-ainchors-grey-light/45 px-2.5 py-1.5 text-xs text-ainchors-navy focus:border-ainchors-green focus:ring-2 focus:ring-ainchors-green/25">
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <form id="{{ $revokeFormId }}" method="POST" action="{{ route('admin.enrollments.revoke', $enrollment) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="font-semibold text-red-700 transition hover:text-red-900 focus:outline-none focus:ring-2 focus:ring-red-500">Revoke<span class="sr-only"> {{ $enrollment->product?->name ?? 'enrollment' }} for {{ $enrollment->user?->full_name ?? 'user' }}</span></button>
                                    </form>
                                </td>
                            @else
                                <td class="px-5 py-4 text-ainchors-grey-light">—</td>
                                <td class="px-5 py-4 text-right"><span class="text-xs font-semibold text-ainchors-grey-light">Already revoked</span></td>
                            @endif
                        </tr>
                    @empty
